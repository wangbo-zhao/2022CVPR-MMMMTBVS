...
# testdataset = A2DTestDataset(para, a2d_dataset_root)
...
model = PropagationModel(para, logger=logger, save_path=save_path)
...
if para['stage'] == 0: # RefCOCO
    ...
    train_dataset, train_sampler, train_loader = renew_loader(para, dataset_root)
    ...
    train_dataset, train_sampler, train_loader = renew_loader(para, max_skip=5)
...
try:
    ...
    for e in range(current_epoch, total_epoch):
        ...
        if para['stage'] != 0 and e != total_epoch and e >= increase_skip_epoch[0]:
            ...
            if para['stage'] != 0:
                train_dataset, train_sampler, train_loader = renew_loader(para=para, max_skip=cur_skip)
        ...
        if total_iter >= 16000:
            ...
            data_loader_tests = [torch.utils.data.DataLoader(test_datasets[k], batch_size=1, sampler=test_samplers[k], num_workers=8) for k in range(len(test_datasets))]

            mean_iou, overall_iou, precision5, precision6, precision7, precision8, precision9, precision_mAP, total_time = evaluate_during_training(args=para, test_datasets=test_datasets, model_test=model_test, data_loader_tests=data_loader_tests, epoch=e,
                                             long_id=long_id, epoch_start_eval=epoch_start_eval, iterations=total_iter, output_dir=output_dir)



            metrics = [total_iter, precision5, precision6, precision7, precision8, precision9,
